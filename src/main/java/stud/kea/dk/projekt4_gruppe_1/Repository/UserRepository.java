...
public class UserRepository {
    ...
    public List<User> getUser(){
        String DISPLAY_USER_SQL = "SELECT FROM user";
        List<User> userList = jdbcTemplate.query(DISPLAY_USER_SQL,new BeanPropertyRowMapper<>(User.class));
        return userList;
    }
    public void createUser(User user) {
        final String INSERT_USER_SQL = "INSERT INTO user (firstname, lastname, email, pwd) VALUES (?,?,?,?)";
        jdbcTemplate.update(INSERT_USER_SQL, user.getFirstName(), user.getLastName(), user.getEmail(), user.getPwd());
    }

    public User getUserListbyId(int id) {
        String get_USER_SQL = "SELECT * FROM user WHERE id =?";
        return jdbcTemplate.queryForObject(get_USER_SQL, new Object[]{id}, new BeanPropertyRowMapper<>(User.class));
    }

    public void deleteUserById(int id) {
        String DELETE_USERLIST_SQL = "DELETE FROM user WHERE id = ?";
        jdbcTemplate.update(DELETE_USERLIST_SQL, id);
    }

    public void updateProducts(User user) {
        String UPDATE_USER_SQL = "UPDATE user SET firstname WHERE id =?";
